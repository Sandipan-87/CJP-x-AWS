...
**2026-08-18 — Session 47 · Repo renamed CJP-x-AWS → ENGRAM (already done on GitHub itself; local remote was still pointing at the old URL); README.md rewritten by merging two drafts, catching and fixing three real inaccuracies along the way.** `gh repo view` showed the GitHub repository had already been renamed to `ENGRAM` (owner `Sandipan-87`), but `git remote -v` still pointed at `https://github.com/.../CJP-x-AWS.git` — updated via `git remote set-url origin` and confirmed connectivity with `git ls-remote`. Searched every tracked file for "CJP" (`git grep -il`) rather than guessing scope: found real references only in `README.md` (clone URL) and `docs/phase0-verification.md` (repo-URL evidence row), both fixed; left CLAUDE.md's own historical changelog entries and `scripts/demo_run.py`'s comment about the local directory path alone, since those describe a real, still-true local filesystem fact (the working directory genuinely has a space in its name, `CJP x AWS`) rather than the project's public name — rewriting historical changelog to pretend otherwise would misrepresent what actually happened. **User supplied a second, independently-drafted README and asked for a merge, explicitly warning against carrying over wrong metrics or insufficient detail from either draft** — rather than merge blindly, verified every claim that differed between the two against the actual code before deciding which to keep. This caught three real inaccuracies in the user's supplied draft, not assumed correct: (1) its Quickstart command `python -m agent.main` is CORRECT and the previously-committed README's own `python agent/main.py` is WRONG — confirmed by actually running both: the script form crashes immediately with `ModuleNotFoundError: No module named 'agent'` (`agent/main.py` uses absolute `agent.*` imports, so it must run as a module with the repo root on `sys.path`), while `-m agent.main` correctly gets past every import and only fails later on a missing `ENGRAM_MEMORY_DSN` env var, exactly as expected without a real `.env`. (2) Its dashboard description ("an Overview page, an Incidents page with per-incident journey timelines, a Memory explorer, an Approvals queue") does not match the real app — checked directly: `dashboard/src/app` has exactly one page (`page.tsx`), rendering five panels (Task Feed, Action Feed, Memory Inspector, Approval Queue, Metrics) on one screen, not four separate pages. Kept the previously-committed README's accurate single-page/five-panel description instead. (3) Its Repository-layout claim that `dashboard/`'s detail pages read from `db/migrations/011_reader_dashboard_v2.sql` is FALSE — `ls db/migrations/` tops out at `010_reader_recall_citations_grant.sql`; no migration 011 exists anywhere in the repo. Dropped the claim rather than inventing the file to match it. Kept the user's draft's genuine improvements: the richer, more detailed architecture diagram (target-cluster probe/operator/backup-gate-REST breakdown), the new "Why this architecture" design-rationale section (memory-is-the-product, two-clusters-two-roles, exactly-once, safety-is-structural — all independently verified against `db/migrations/`, `agent/graph.py`, and the IAM tables, not just copied), and its more honest CockroachDB-tools framing for the Managed MCP Server (measured in Phase 0, deliberately kept off the hot path) — but rewrote its confusing "this is the control-plane integration the ccloud CLI wraps" sentence, since the real point (discovered this same session while drafting the Devpost answers) is the opposite: `ccloud`'s own `cluster` subcommand set has no `backup list` at all, which is *why* the backup gate had to bypass the CLI and call the Cloud REST API directly. Re-verified the "226 unit tests" claim by actually running `pytest --collect-only` (226 collected, exact, not "226+") rather than trusting either draft's number. Added one new measured-numbers row ("Final demo recording — incident #1, applied fix: 1400.0ms → 4.0ms, friend cluster") reflecting the user's own reported number from the actual final recording take, kept as its own row rather than silently merged into the pre-existing, differently-sourced "Kill-and-resume, applied fix" row of the same value, since they're separate real measurements (different cluster, different incident) that happen to coincide numerically. `docs/phase0-verification.md`'s repo-URL evidence row updated to the new URL with a note that it was renamed. Committed and pushed.
