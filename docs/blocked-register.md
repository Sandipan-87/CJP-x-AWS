...
## 7 — S3 bucket `engram-agent-artifacts` · [PLUMBER] · **RESOLVED 2026-08-11 — bucket created, IAM policy attached, gate PASSES**

**Symptom 1 (RESOLVED).** `scripts/verify_s3.py` authenticated correctly as `arn:aws:iam::532749777349:user/engram-phase0` but `PutObject` failed `NoSuchBucket` in `us-east-1` — the bucket was named in design docs and `.env` but never provisioned. A direct test confirmed the IAM identity correctly **lacks** `s3:CreateBucket` — least-privilege working as designed, not a misconfiguration. **Fix applied:** user created the bucket via the AWS console.

**Symptom 2 (RESOLVED).** Re-running got one step further and failed differently: `AccessDenied` on `PutObject` — bucket creation and IAM policy are two separate AWS actions; only the bucket had been provisioned. **Fix applied:** user attached an inline policy to `engram-phase0` granting `s3:PutObject` + `s3:GetObject` scoped to `arn:aws:s3:::engram-agent-artifacts/*` — never `s3:*`, never `Resource: "*"` (invariant #11's own constraint).
...
## 8 — Backup gate (`agent/tools/cloud_api.py`) has no live credential · [PLUMBER] · **RESOLVED 2026-08-11 (Session 29)**

**Symptom (original).** LLD §5.5 step 1's backup gate needs `GET /api/v1/clusters/{id}/backups` on the CockroachDB Cloud REST API, authenticated with a Cluster-Admin-scoped key (LLD §2: `CCLOUD_TOKEN`). No such key — or any Cloud API credential at all — existed in `.env`.

**Session 28 update.** `ENGRAM_TARGET_PROBE_DSN`/`ENGRAM_TARGET_OPERATOR_DSN` — the sibling gap this row used to be grouped with — were resolved first (`db/target/001_target_roles.sql` + `scripts/bootstrap_target_roles.py`). `CCLOUD_TOKEN` needed a human at the CockroachDB Cloud console instead — not something reachable from a SQL connection or REST call.

**Session 29 — RESOLVED.** The user provisioned a Service Account key via the console. **First attempt failed with a real, instructive error**: `scripts/verify_ccloud.py` got `401 {"code": 16, "message": "invalid secret provided in authorization header; expected either an API key or a JWT"}` on BOTH clusters — a structural check (36 chars, `[0-9a-f]{8}-[0-9a-f]{4}-...` UUID shape, zero dots) confirmed the pasted value was the Service Account's **Client ID**, not its **Client Secret/API key** — an easy console mistake since both are shown together at creation time. Once corrected to the actual secret, **`scripts/verify_ccloud.py` passed 3/3, first real run**: `200` on the target cluster, a genuine `403 {"code": 7, "message": "unauthorized"}` on the memory cluster (proving the key is correctly scoped to target-only, the OPPOSITE of the 2026-08-03 wrong-scope mistake this file used to warn about), and a real non-empty backups list.

**A second real bug surfaced by this first-ever non-empty response, fixed same session.** The actual response shape is `{"backups": [{"id": <uuid>, "as_of_time": <ISO8601>}, ...], "pagination": null}` — the completion-timestamp field is **`as_of_time`**, not `completedTime`/`completed_at`/`finishedTime` (the pre-measurement best guess `decide_backup_gate()` used to check first). Captured as `fixtures/cloudapi-backups-target-nonempty.json` (real evidence, committed, no secrets in it — just backup IDs and timestamps) alongside the existing empty-list fixture. `agent/tools/cloud_api.py`'s `decide_backup_gate()` now checks `as_of_time` first, keeping the old guesses as fallbacks rather than removing them outright. `tests/test_cloud_api.py` grew 4 new tests against the real non-empty fixture (16/16 total, up from 12/12) — including one proving the fixture itself never regresses to the old, wrong field name.

**What remains open, stated not hidden.** `agent/nodes/act_measure.py`'s own smoke test still uses `override_backup_gate=True` (the LLD-named audited escape hatch) rather than the real network path — wiring a real `CloudApiAdapter` through an actual `act_measure` run is real follow-up work, not done in this session. The GitHub Actions `CCLOUD_TOKEN` repo secret (for the CI path) also hasn't been added yet — only the local `.env` value has been verified.
