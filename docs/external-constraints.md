...
- **VERIFIED 2026-08-11** (`scripts/verify_s3.py`, gate PASS): bucket created via console, then a scoped IAM policy attached (`s3:PutObject`+`s3:GetObject` on `arn:aws:s3:::engram-agent-artifacts/*`, never `s3:*`) — auth, put, and get/hash all measured working, a real object round-tripped with a byte-identical sha256. The IAM identity (`arn:aws:iam::532749777349:user/engram-phase0`) was, throughout, confirmed by direct test to **lack** `s3:CreateBucket` — least-privilege enforced exactly as intended, not worked around. Cleanup (`DeleteObject`) was never granted and correctly fails — a deliberate scope choice, not a gap. Full transcript: `docs/phase0-verification.md` §3.4–§3.6; diagnosis: `docs/blocked-register.md` §7.
